...
ERP 스타일 BOM 원가 계산 시스템에서 `BOM` 생성/수정 시마다 `bom_cost_cache`를 자동 재빌드하도록 설계한 결과, 데이터 일관성 붕괴, 대량 처리 시 성능 저하, SRP(단일 책임 원칙) 및 트랜잭션 경계 위반이 동시에 드러났다. 동일 시점에 `bom_closure`는 362행, `bom_cost_cache`는 130행만 존재하는 정량적 불일치를 통해, BOM 구조 변경을 원가 계산의 자동 트리거로 사용하는 설계가 ERP 도메인에서는 근본적으로 잘못되었다는 점이 확인되었다. 이를 계기로 BOM 구조 변경은 구조 정의 작업으로, 원가 계산은 명시적 비즈니스 연산으로 분리하고, `bom_cost_cache`를 웹 캐시가 아닌 결산 기준 계산 결과 저장소로 재정의하는 아키텍처 결정을 내렸다.
