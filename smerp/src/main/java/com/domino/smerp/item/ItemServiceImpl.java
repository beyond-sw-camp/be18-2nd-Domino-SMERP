...
public class ItemServiceImpl implements ItemService {
    ...
    public PageResponse<ItemListResponse> searchItems(final ItemSearchRequest keyword, final Pageable pageable) {
        return PageResponse.from(
                itemRepository
                        .searchItems(keyword, pageable) // Page<Item>
    ...
    @Override
    @Transactional
    public ItemStatusResponse updateItemStatus(final Long itemId, final UpdateItemStatusRequest request) {
        Item item = findItemById(itemId);
    ...
    public ItemStatus findItemStatusById(final Long itemStatusId) {
        return itemStatusRepository
                .findById(itemStatusId)
                .orElseThrow(() -> new CustomException(ErrorCode.ITEM_STATUS_NOT_FOUND));
    }

    // 품목 findById
    public Item findItemById(final Long itemId) {
        return itemRepository.findById(itemId).orElseThrow(() -> new CustomException(ErrorCode.ITEM_NOT_FOUND));
    }
