...
public class ItemController {
    ...
    // 품목 생성
    @PostMapping
    public ResponseEntity<ItemDetailResponse> createItem(final @Valid @RequestBody CreateItemRequest request) {
    ...
    public ResponseEntity<PageResponse<ItemListResponse>> getItems(
            @ModelAttribute ItemSearchRequest keyword, Pageable pageable) {
        return ResponseEntity.ok(itemService.searchItems(keyword, pageable));
    }

    // 품목 상세 조회
    @GetMapping("/{item-id}")
    public ResponseEntity<ItemDetailResponse> getItemById(@PathVariable("item-id") final Long itemId) {
    ...
    public ResponseEntity<ItemDetailResponse> updateItem(
            @PathVariable("item-id") final Long itemId, final @Valid @RequestBody UpdateItemRequest request) {
    ...
    public ResponseEntity<ItemStatusResponse> updateItemStatus(
            @PathVariable("item-id") final Long itemId, final @Valid @RequestBody UpdateItemStatusRequest request) {
